...
async function cxnDB(){
  ...
  try{
    await client.connect().then(
      client.db("quebecdb").collection("genres")
      );       
  }
  catch(e){
      console.log(e)
...
app.get('/', async (req, res) => {
 ...
 client.connect;
  let mongoResult = await client.db("quebecdb").collection("genres").find().toArray();
...
app.post('/updateProfile', async (req, res) => {
  ...
  try {
    ...
    console.log("user Name: ", req.body.genreName)
    
    client.connect; 
    const collection = client.db("quebecdb").collection("genres");
  
    // put it into mongo
    let result = await collection.findOneAndUpdate( 
      { _id: new ObjectId( req.body.genreId ) },
